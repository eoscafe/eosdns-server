...
class Compressor {
  ...
  add (str) {
    ...
    for (const [word, part] of parts) {
      ...
      if (this.words.length === 0x7f) { continue }
  ...
  getSize () {
    let size = 1
    for (const word of this.words) { size += 1 + word.length }
    return size
  }
...
class Decompressor {
  ...
  _readString (data, off, max = 255) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (!isCompressedCh(ch)) { throw new Error('Non-printable character.') }
      ...
      if (ch & 0x80) {
        ...
        if (str.length > max) { throw new Error('String too large.') }

        last = i + 1
      }
    }

    if (last !== len) { str += data.toString('ascii', off + last, off + len) }
  ...
  _readBytes (data, off, max = 255) {
    ...
    if (last !== len) { size += len - last }

    if (size > max) { throw new Error('String too large.') }
    ...
    if (last !== len) { pos += data.copy(str, pos, off + last, off + len) }
  ...
  read (br) {
    ...
    for (let i = 0; i < count; i++) { this.words.push(readAscii(br, br.readU8())) }
...
function isWordCh (ch) {
  ...
  // - and _
  if (ch === 0x2d || ch === 0x5f) { return true }
...
function splitString (str) {
  ...
  for (; i < str.length; i++) {
    ...
    if (word === !isWordCh(ch)) {
      if (i !== last) { parts.push([word, str.substring(last, i)]) }
      word = !word
      last = i
    }
  }
...
function readAscii (br, size) {
  ...
  for (let i = 0; i < size; i++) {
    if (offset + i >= data.length) { throw new Error('Out of bounds read.') }

    const ch = data[offset + i]

    // Any non-printable character can screw.
    if (!isStringCh(ch)) { throw new Error('Non-printable character.') }
  }
