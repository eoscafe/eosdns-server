...
class Target extends Struct {
  ...
  from (type, target) {
    if (typeof type === 'string') { return this.fromString(type) }
  ...
  toString () {
    if (this.isGlue()) {
      if (this.inet4 === '0.0.0.0' && this.inet6 === '::') { throw new Error('Bad glue address.') }

      const ips = []

      if (this.inet4 !== '0.0.0.0') { ips.push(this.inet4) }

      if (this.inet6 !== '::') { ips.push(this.inet6) }
  ...
  fromString (str) {
    ...
    if (parts.length > 1) {
      ...
      for (const ip of ips) {
        ...
        case types.INET4:
          if (addr === '0.0.0.0') { throw new Error('Bad glue address.') }
          this.inet4 = addr
          break
        case types.INET6:
          if (addr === '::') { throw new Error('Bad glue address.') }
          this.inet6 = addr
          break
        }
      }
  ...
  static isPointer (name, type) {
    ...
    if (name.length < 2 || name.length > 29) { return false }
...
class Service extends Struct {
  ...
  read (br) {
    ...
    if (util.countLabels(this.service) !== 1) { throw new Error('Invalid label.') }

    this.protocol = readNameBR(br)

    if (util.countLabels(this.protocol) !== 1) { throw new Error('Invalid label.') }
  ...
  fromJSON (json) {
    ...
    if (json.target != null) { this.target.fromJSON(json.target) }
...
class Magnet extends Struct {
  ...
  read (br) {
    ...
    if (util.countLabels(this.nid) !== 1) { throw new Error('Invalid label.') }
...
class Addr extends Struct {
  ...
  getSize () {
    ...
    if (util.equal(this.currency, 'ethereum.')) { return 1 + 20 }
...
function parseIP (str) {
  ...
  if (IP.isIPv4(ip)) { return [types.INET4, IP.toString(ip)] }

  if (IP.isIPv6(ip)) { return [types.INET6, IP.toString(ip)] }

  if (IP.isOnion(ip)) { return [types.ONION, IP.toString(ip)] }
...
function isSingle (label) {
  ...
  if (!isName(label)) { return false }

  if (util.countLabels(label) !== 1) { return false }
