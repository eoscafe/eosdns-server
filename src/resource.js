...
class Resource extends Struct {
  ...
  compress () {
    ...
    for (const host of this.hosts) { host.compress(c) }

    if (this.canonical) { this.canonical.compress(c) }

    if (this.delegate) { this.delegate.compress(c) }

    for (const ns of this.ns) { ns.compress(c) }

    for (const srv of this.service) { srv.compress(c) }

    for (const uri of this.uri) { c.add(uri) }

    for (const email of this.email) { c.add(email) }

    for (const text of this.text) { c.add(text) }

    for (const loc of this.location) { loc.compress(c) }

    for (const urn of this.magnet) { urn.compress(c) }

    for (const ds of this.ds) { ds.compress(c) }

    for (const tls of this.tls) { tls.compress(c) }

    for (const smime of this.smime) { smime.compress(c) }

    for (const ssh of this.ssh) { ssh.compress(c) }

    for (const pgp of this.pgp) { pgp.compress(c) }

    for (const addr of this.addr) { addr.compress(c) }

    for (const extra of this.extra) { extra.compress(c) }
  ...
  getSize (c) {
    ...
    for (const host of this.hosts) { size += host.getSize(c) }

    if (this.canonical) { size += 1 + this.canonical.getSize(c) }

    if (this.delegate) { size += 1 + this.delegate.getSize(c) }

    for (const ns of this.ns) { size += 1 + ns.getSize(c) }

    for (const srv of this.service) { size += 1 + srv.getSize(c) }

    for (const uri of this.uri) { size += 1 + c.size(uri) }

    for (const email of this.email) { size += 1 + c.size(email) }

    for (const text of this.text) { size += 1 + c.size(text) }

    for (const loc of this.location) { size += 1 + loc.getSize(c) }

    for (const urn of this.magnet) { size += 1 + urn.getSize(c) }

    for (const ds of this.ds) { size += 1 + ds.getSize(c) }

    for (const tls of this.tls) { size += 1 + tls.getSize(c) }

    for (const smime of this.smime) { size += 1 + smime.getSize(c) }

    for (const ssh of this.ssh) { size += 1 + ssh.getSize(c) }

    for (const pgp of this.pgp) { size += 1 + pgp.getSize(c) }

    for (const addr of this.addr) { size += 1 + addr.getSize(c) }

    for (const extra of this.extra) { size += 1 + extra.getSize(c) }
  ...
  write (bw, c) {
    ...
    for (const host of this.hosts) { host.write(bw, c) }
  ...
  read (br) {
    ...
    if (version !== 0) { throw new Error(`Unknown serialization version: ${version}.`) }
    ...
    while (br.left()) {
      if (count === 255) { throw new Error('Too many records.') }
  ...
  toA (name) {
    ...
    if (this.hasTor()) { answer.push(this.toTorTXT(name)) }
  ...
  toNS (name) {
    ...
    for (const ns of this.ns) {
      ...
      if (ns.isName()) { nsname = ns.toDNS() } else if (ns.isINET()) { nsname = ns.toPointer(name) }
  ...
  toMX (name) {
    ...
    for (const srv of this.service) {
      if (!util.equal(srv.protocol, 'tcp.')) { continue }

      if (!util.equal(srv.service, 'smtp.')) { continue }

      let mxname = null

      if (srv.target.isName()) { mxname = srv.target.toDNS() } else if (srv.target.isINET()) { mxname = srv.target.toPointer(name) }
  ...
  toSRV (name, protocol, service) {
    ...
    for (const srv of this.service) {
      ...
      if (srv.target.isName()) { target = srv.target.toDNS() } else if (srv.target.isINET()) { target = srv.target.toPointer(name) }

      if (!target) { continue }

      if (!util.equal(protocol, srv.protocol)) { continue }

      if (!util.equal(service, srv.service)) { continue }
  ...
  toTLSA (name, protocol, port) {
    ...
    for (const tls of this.tls) {
      ...
      if (!util.equal(protocol, tls.protocol)) { continue }

      if (tls.port !== port) { continue }
  ...
  toSMIMEA (name, hash) {
    ...
    for (const smime of this.smime) {
      ...
      if (!hash.equals(smime.hash)) { continue }
  ...
  toOPENPGPKEY (name, hash) {
    ...
    for (const pgp of this.pgp) {
      ...
      if (!hash.equals(pgp.hash)) { continue }
  ...
  toRP (name) {
    ...
    for (const email of this.email) {
      ...
      if (!util.isName(rd.mbox)) { continue }
  ...
  toTorTXT (name) {
    ...
    for (const host of this.hosts) {
      if (host.isTor()) { rd.txt.push(host.target) }
    }
  ...
  toGlue (type) {
    ...
    switch (type) {
    case types.CNAME:
      if (this.canonical && this.canonical.isGlue()) { glue.push(this.canonical) }
      break
    case types.DNAME:
      if (this.delegate && this.delegate.isGlue()) { glue.push(this.delegate) }
      break
    case types.NS:
      for (const ns of this.ns) {
        if (ns.isGlue()) { glue.push(ns) }
      }
      break
    case types.SRV:
    case types.MX:
      for (const srv of this.service) {
        ...
        if (srv.target.isGlue()) { glue.push(srv.target) }
      }
  ...
  toDNS (name, type) {
    ...
    if (labels.length === 3) {
      ...
      case types.SRV: {
        ...
        if (ok) {
          ...
          for (const rr of this.toGlue(types.SRV)) { res.additional.push(rr) }
    ...
    if (labels.length > 1) {
      ...
      if (this.ns.length > 0) {
        ...
        for (const rr of this.toDS(tld)) { res.authority.push(rr) }

        res.additional = this.toNSIP(tld)

        for (const rr of this.toGlue(types.NS)) { res.additional.push(rr) }

        if (this.ds.length === 0) { key.signZSK(res.authority, types.NS) } else { key.signZSK(res.authority, types.DS) }
      } else if (this.delegate) {
        res.answer = this.toDNAME(name)

        for (const rr of this.toGlue(types.DNAME)) { res.additional.push(rr) }
    ...
    case types.CNAME:
      ...
      for (const rr of this.toGlue(types.CNAME)) { res.additional.push(rr) }
    ...
    case types.MX:
      ...
      for (const rr of this.toGlue(types.MX)) { res.additional.push(rr) }
    ...
    if (res.answer.length > 0) { res.aa = true }
    ...
    if (res.answer.length === 0 &&
      ...
      if (this.canonical) {
        ...
        for (const rr of this.toDS(name)) { res.authority.push(rr) }
  ...
  getJSON (name) {
    ...
    if (this.hosts.length > 0) {
      json.hosts = []
      for (const host of this.hosts) { json.hosts.push(host.toJSON()) }
    }

    if (this.canonical) { json.canonical = this.canonical.toJSON() }

    if (this.delegate) { json.delegate = this.delegate.toJSON() }

    if (this.ns.length > 0) {
      json.ns = []
      for (const ns of this.ns) { json.ns.push(ns.toJSON()) }
    }

    if (this.service.length > 0) {
      json.service = []
      for (const srv of this.service) { json.service.push(srv.toJSON()) }
    }

    if (this.uri.length > 0) {
      json.uri = []
      for (const uri of this.uri) { json.uri.push(uri) }
    }

    if (this.email.length > 0) {
      json.email = []
      for (const email of this.email) { json.email.push(email) }
    }

    if (this.text.length > 0) {
      json.text = []
      for (const txt of this.text) { json.text.push(txt) }
    }

    if (this.location.length > 0) {
      json.location = []
      for (const loc of this.location) { json.location.push(loc.toJSON()) }
    }

    if (this.magnet.length > 0) {
      json.magnet = []
      for (const urn of this.magnet) { json.magnet.push(urn.toJSON()) }
    }

    if (this.ds.length > 0) {
      json.ds = []
      for (const ds of this.ds) { json.ds.push(ds.toJSON()) }
    }

    if (this.tls.length > 0) {
      json.tls = []
      for (const tls of this.tls) { json.tls.push(tls.toJSON()) }
    }

    if (this.smime.length > 0) {
      json.smime = []
      for (const smime of this.smime) { json.smime.push(smime.toJSON()) }
    }

    if (this.ssh.length > 0) {
      json.ssh = []
      for (const ssh of this.ssh) { json.ssh.push(ssh.toJSON()) }
    }

    if (this.pgp.length > 0) {
      json.pgp = []
      for (const pgp of this.pgp) { json.pgp.push(pgp.toJSON()) }
    }
    ...
    if (this.addr.length > 0) {
      json.addr = []
      for (const addr of this.addr) { json.addr.push(addr.toJSON()) }
    }

    if (this.extra.length > 0) {
      json.extra = []
      for (const extra of this.extra) { json.extra.push(extra.toJSON()) }
    }
  ...
  fromJSON (json) {
    ...
    if (json.hosts != null) {
      ...
      for (const host of json.hosts) { this.hosts.push(Target.fromJSON(host)) }
    }
    ...
    if (json.service != null) {
      ...
      for (const srv of json.service) { this.service.push(Service.fromJSON(srv)) }
    }
    ...
    if (json.location != null) {
      ...
      for (const loc of json.location) { this.location.push(Location.fromJSON(loc)) }
    }

    if (json.magnet != null) {
      assert(Array.isArray(json.magnet))
      for (const urn of json.magnet) { this.magnet.push(Magnet.fromJSON(urn)) }
    }

    if (json.ds != null) {
      assert(Array.isArray(json.ds))
      for (const ds of json.ds) { this.ds.push(DS.fromJSON(ds)) }
    }

    if (json.tls != null) {
      assert(Array.isArray(json.tls))
      for (const tls of json.tls) { this.tls.push(TLS.fromJSON(tls)) }
    }

    if (json.smime != null) {
      assert(Array.isArray(json.smime))
      for (const smime of json.smime) { this.smime.push(SMIME.fromJSON(smime)) }
    }

    if (json.ssh != null) {
      assert(Array.isArray(json.ssh))
      for (const ssh of json.ssh) { this.ssh.push(SSH.fromJSON(ssh)) }
    }

    if (json.pgp != null) {
      assert(Array.isArray(json.pgp))
      for (const pgp of json.pgp) { this.pgp.push(PGP.fromJSON(pgp)) }
    }

    if (json.addr != null) {
      assert(Array.isArray(json.addr))
      for (const addr of json.addr) { this.addr.push(Addr.fromJSON(addr)) }
    }

    if (json.extra != null) {
      assert(Array.isArray(json.extra))
      for (const extra of json.extra) { this.extra.push(Extra.fromJSON(extra)) }
    }
